# US-052 - Sort My Tasks  ## Metadata - Area: 15. My Tasks - GitHub labels: `user-story`, `mvp`, `area:my-tasks` - Suggested status: `Backlog` - Suggested wave: `Wave 5` - Depends on: US-051 - Parallelization note: Start once dependencies are done; run in parallel with other stories in the same wave that do not share blocking dependencies.  ## User Story **As a** user  
...
**Then** the relevant indicator is shown.  ## Implementation Breakdown **Kanban lane:** Backlog â†’ Ready â†’ Red â†’ Green â†’ Refactor â†’ Review / QA â†’ Done  
