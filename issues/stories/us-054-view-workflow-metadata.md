# US-054 - View Workflow Metadata  ## Metadata - Area: 16. Priorities and Metadata - GitHub labels: `user-story`, `mvp`, `area:metadata` - Suggested status: `Backlog` - Suggested wave: `Wave 5` - Depends on: US-028, US-053 - Parallelization note: Start once dependencies are done; run in parallel with other stories in the same wave that do not share blocking dependencies.  ## User Story **As a** frontend application  
...
**Then** statuses, transitions, and priorities are returned from the database.  ## Implementation Breakdown **Kanban lane:** Backlog â†’ Ready â†’ Red â†’ Green â†’ Refactor â†’ Review / QA â†’ Done  
