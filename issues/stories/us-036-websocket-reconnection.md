# US-036 - WebSocket Reconnection  ## Metadata - Area: 9. Comments and Real-Time Updates - GitHub labels: `user-story`, `mvp`, `area:comments` - Suggested status: `Backlog` - Suggested wave: `Wave 4` - Depends on: US-035 - Parallelization note: Start once dependencies are done; run in parallel with other stories in the same wave that do not share blocking dependencies.  ## User Story **As a** user  
...
**Then** the frontend fetches missed comments since the last received comment.  ## Implementation Breakdown **Kanban lane:** Backlog â†’ Ready â†’ Red â†’ Green â†’ Refactor â†’ Review / QA â†’ Done  
